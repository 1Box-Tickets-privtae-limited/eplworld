        ...
        <div class="row">
                                            ...
                                            <div class="col-md-12 nopad">
                                                <div class="popular-tickets-section">
                                                    <div class="team-tickets">
                                                        <ul>
                                                            <li><a
                                                                    href="{{ url(app()->getLocale()) . '/' . $list['team_slug_a'] }}">
                                                                    <img src="{{ $list['team_image_a'] }}"
                                                                        alt="{{ $list['team_name_a'] }}" >
                                                                </a></li>
                                                            <li><a
                                                                    href="{{ url(app()->getLocale()) . '/' . $list['team_slug_b'] }}">
                                                                    <img src="{{ $list['team_image_b'] }}"
                                                                        alt="{{ $list['team_name_b'] }}">
                                                                </a></li>
                                                        </ul>
                                                    </div>
                                                    <div class="team-tickets-content">
                                                        <a href="{{ url(app()->getLocale()) . '/' . $list['slug'] }}">
                                                            <h2>{{ $list['team_name_a'] }}<span>vs</span>{{ $list['team_name_b'] }}
                                                            </h2>
                                                        </a>
                                                        <div class="popular-date-time">{{ $list['match_date'] }} |
                                                            {{ $list['match_time'] }}</div>
                                                        <p class="stad_name_show">{{ $list['stadium_name'] }}</p>
                                                        <p><span>{{ $list['tournament_name'] }}</span></p>
                                                        <div class="popular-view-btn">
                                                            <a
                                                                href="{{ url(app()->getLocale()) . '/' . $list['slug'] }}">{{ __('messages.view tickets') }}</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
            ...
            <div class="col-md-12">
                <div class="upcoming-match-btn-view-all">
                    <a href="{{ url(app()->getLocale()) . '/all-games-tickets' }}" class="onebox-btn">{{ __('messages.See All Matches') }}</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row">
            ...
            @if ($top_teams)
                @foreach ($top_teams as $list)
                    <div class="col-md-3 col-sm-6 col-xs-6 full_widd">
                        <div class="top_teams top_teams_home">
                            ...
                            <div class="top_team_details">
                                <div class="top_team_img_log">
                                    <a class="no-underline"
                                        href="{{ url(app()->getLocale()) . '/' . $list['url_key'] }}"><img
                                            src="{{ $list['team_image'] }}" alt="{{ $list['team_name'] }}"></a>
                                </div>
                                <div class="top_team_cont">
                                    <a class="no-underline"
                                        href="{{ url(app()->getLocale()) . '/' . $list['url_key'] }}">
                                    ...
                                    <div class="team_match_status">
                                        @if ($list['match_name'])
                                            ...
                                            <p> <a href="{{ url(app()->getLocale()) . '/' . $list['match_slug'] }}">{{ @$list['team_name_a'] }}
                                                    <span>vs</span> {{ @$list['team_name_b'] }}</a></p>
                                        @endif
            ...
            <div class="col-md-12">
                <div class="upcoming-match-btn-view-all">
                    ...
                    <a href="{{ url(app()->getLocale() . '/teams') }}"
                        class="onebox-btn">{{ __('messages.view all teams') }}</a>
                </div>
            </div>
...
<section class="epl-popular-teams section_50">
    <div class="container">
                                    ...
                                    <div class="all_ticket_list">
                                        <div class="event_lists">
                                            @if ($list['total_match'] > 0)
                                                <h4>{{ $list['total_match'] }}
                                                    {{ $list['total_match'] > 1 ? __('messages.events tournaments') : __('messages.event') }}
                                                </h4>
                                            @endif
                                            @if ($list['ticket_available'] > 0)
                                                <p><span class="span_ltr">{{ $list['ticket_available'] }}</span>
                                                    {{ $list['ticket_available'] > 1 ? __('messages.tickets listed') : __('messages.ticket listed') }}</span>
                                                </p>
                                            @else
                                                <p> &nbsp;</p>
                                            @endif
                                        ...
                                        <div class="ticket_starting">
                                            ...
                                            @elseif($list['request_type'] == 'book')
                                                <h4>{{ __('messages.from') }} <span
                                                        class="span_ticket_val">{{ $list['currency_symbol'] . ' ' . $list['ticket_price'] }}</span>
                                                </h4>
        ...
        <div class="col-md-12">
            <div class="upcoming-match-btn-view-all">
                <a href="{{ url(app()->getLocale() . '/tournament-tickets/') }}"
                    class="onebox-btn">{{ __('messages.show more tournaments') }}</a>
            </div>
        </div>
    </div>
</div>
