@extends('layouts.app')
@section('content')
<link rel="stylesheet" type="text/css" href="{{asset('/')}}/public/css/svgmap.css"><style type="text/css">
            ...
            <div class="last-match-box">
                    ...
                    <div class="last-match-result-one last-match-result">
                            <div class="result-details">
                                <div class="last-match-logo">
                                    <a href="{{url(app()->getLocale().'/')}}/{{$results['team_slug_a']}}"><img src="{{$results['team_image_a']}}" style="height:140px" alt="logo"></a>
                ...
                <div class="col-md-6 col-sm-6 col-xs-6 pad_five">
                    <div class="last-match-score match_ticket_selection">
                        <h1>{{$results['team_name_a']}}<span>vs</span> {{$results['team_name_b']}}</h1>
                        @if(@$results['match_label'])<p  class="match_name_orginal"style="font-size:14px;margin: 10px" >{{@$results['match_label']}}</p>
                        @endif
                        <h2>{{$results['tournament_name']}}</h2>
                ...
                <div class="col-md-3 col-sm-3 col-xs-3">
                    <div class="last-match-result-one last-match-result">
                            <div class="result-details">
                                <div class="last-match-logo">
                                    <a href="{{url(app()->getLocale().'/')}}/{{$results['team_slug_b']}}"><img src="{{$results['team_image_b']}}" style="height:140px" alt="logo"></a>
                                    <h3 class="result-details-left">
        ...
        <div class="row column_mobile">
            <div class="col-md-5 position_stct" style="position: sticky; top: 20px;" >
                ...
                <div class="ticket_select_img">
                    <a href="javascript:void(0)" class="reset_stadium reset_filter">Reset Filter</a>
                    <div id="mapsvg"></div>
                ...
                <div class="stadium_para_notes">
                    ...
                    <ul>
                        <li> <i class="fas fa-check"></i>{{__('messages.event date and time are subject to change - check with the venue for start times and/or age restrictions.')}}</li>
                        <li> <i class="fas fa-check"></i>{{__('messages.all sales are final')}}</li>
                        <li> <i class="fas fa-check"></i>{{__('messages.no cancellations')}}</li>
                        <li> <i class="fas fa-check"></i>{{__('messages.100 ticket guarantee')}}</li>
                    </ul>
...
<!-- jQuery -->
    <script src="{{asset('/')}}/public/js/test_mapsvg.js?ver=2.3.1.2.1.11"></script>
    ...
    get_ticket(0)
     ...
     function get_ticket(cat){
        ...
        $.ajax({
            ...
            type: "post",
            dataType: "json",
            data: {"quantity":qty, "category":cat,"match_id":{{$results['m_id']}},"block_id":block_id,"_token":"{{ csrf_token() }}"},
            success: function(response) {
