...
(function ($) {
    ...
    var defaults = {
        ...
        svgFileVersion: Date.now() + Math.random() ,
    ...
    mapSVG = function (elem, options) {
        ...
        this.methods = {
            ...
            regionClickHandler: function (e, region, skipPopover) {
                ...
                if (region.id_no_spaces && region.id_no_spaces) {
                    tip  = region.id_no_spaces;
                console.log(region.href);
     
                    $('#noofticket').val('0');
                   
                        var ths_cat=region.href ;
                        var sn_exst=0;
                        var ths_cat2="0";
...
//                        window.location.href = region.href;
                                 ...
                                 $("[data-blockid='"+tip+"']").show().siblings('.seat_select_items').not($("[data-blockid='"+tip+"']")).hide();
                                    ...
                                    $(".mapsvg-region").each(function () {
                                        ...
                                        if (tip) {
                                            ...
                                            $('#' + tip).css({fill: stadium_active_block_details[tip]});
                                 
                                            //console.log(darken(stadium_block_details[tip],1);


                                              //    $('#' + tip).css({fill: MapSVG.tinycolor(stadium_block_details[tip]).darken().toRgb()});
                    ...
                    if($("#"+tip).data("cat") in ticket_price_info){
                        ...
                        if(tip in ticket_price_info[$("#"+tip).data("cat")]){
                            if(parseInt(ticket_price_info[$("#"+tip).data("cat")][tip]['no_of_ticket'])>0){
                                ...
                                // _data.tooltip.container.html("Block "+ blockId + '<br/> ' + totalTicketvalue + ' tickets from <br> ' + convertCurrency(Math.min.apply(Math,ticket_price_info[$("#"+tip).data("cat")][tip]['price']))+"<br><br><strong>"+ticket_price_info_with_cat[$("#"+tip).data("cat")]['name']+":</strong><br> Total tickets "+ticket_price_info_with_cat[$("#"+tip).data("cat")]['no_of_ticket']+" from "+convertCurrency(ticket_price_info_with_cat[$("#"+tip).data("cat")]['price']));


                                 _data.tooltip.container.html("Block : "+ blockId + "<br/> Total tickets "+ticket_price_info[$("#"+tip).data("cat")][tip]['no_of_ticket'].reduce((a, b) => a + b, 0)+" from <br>"+convertCurrency(ticket_price_info[$("#"+tip).data("cat")][tip]['price'][0]));

                               //    _data.tooltip.container.html("Total tickets "+ticket_price_info[$("#"+tip).data("cat")]['no_of_ticket']+" from <br><span style='direction:ltr;display: inline-block;'>"+convertCurrency(ticket_price_info [$("#"+tip).data("cat")]['price']+"</span>"));
                                  
                            }
                            else{
                                // _data.tooltip.container.html("<strong>"+ticket_price_info_with_cat[$("#"+tip).data("cat")]['name']+":</strong><br> Total tickets "+ticket_price_info_with_cat[$("#"+tip).data("cat")]['no_of_ticket']+" from <br><span style='direction:ltr;display: inline-block;'>"+convertCurrency(ticket_price_info_with_cat[$("#"+tip).data("cat")]['price']+"</span>"));

                            _data.tooltip.container.html("Total tickets "+ticket_price_info_with_cat[$("#"+tip).data("cat")]['no_of_ticket']+" from <br><span style='direction:ltr;display: inline-block;'>"+convertCurrency(ticket_price_info_with_cat[$("#"+tip).data("cat")]['price']+"</span>"));
            ...
            onClickHandler: function(e,mapsvg){
                ...
                //Custom code by senthil
                    ...
                    $.each(stadium_cat_details,function(indx, itm){
                        if(current_category > 0){
                            var tip_my = $('#ticket_block_id').val();
                             if (tip_my) {
                                    ...
                                    $(".mapsvg-region").each(function () {
                                        //$(this).css('opacity', '1');//
                                        if (tip_my) {
                                            $('#' + tip_my).css('opacity', '1');
                                            $('#' + tip_my).css({fill: stadium_block_details[tip_my]});
                                             $('#' + tip_my).css({fill: stadium_active_block_details[tip_my]});
                                        }
                                    });
            ...
            mouseOutHandler: function (e) {
                ...
                //Custom code by senthil
               // console.log("ccc---" + current_category);
                if(current_category==0 || current_category=="0"){
                   // console.log(stadium_cat_details);
                    $.each(stadium_cat_details,function(indx, itm){                 
                        $.each(itm,function(indx2, itm2){
                            //$('#'+itm2).css('opacity', '1');
                            $('#'+itm2).css({fill: stadium_block_details[itm2]});
            ...
            init: function (opts, elem) {
               ...
               /* if (opts.source.indexOf('//') === 0)
                    ...
                    opts.source = opts.source.replace(/^.*:\/\/[^\/]+/, '').replace('//', '/');
                */
                //console.log('https://www.listmyticket.com/uploads/stadium/'+opts.source);
                /** Setting _data **/
                _data = {};
                ...
                $.ajax({url: _data.options.source + '?v=' + _data.options.svgFileVersion})
                        .done(function (xmlData) {
                            ...
                            _this.setZoom({on: true, limit: [0, 5], delta: 2 ,buttons: {on: true, location: 'left'},mousewheel : false});
                            _this.setScroll({on: true});


                            var regionForm = $('#table-regions').find('#mapsvg-region-' + _this.id_no_spaces);
